...
namespace ya
{
	...
	Scene* Camera::mRootaByssScene = nullptr;
	Scene* Camera::mEastGardenScene = nullptr;
	Scene* Camera::mHeneSisScene = nullptr;
	Scene* Camera::mBanBanScene = nullptr;
	...
	Vector3 Camera::GetWorldPos(Vector3 pos)
	{
		RECT rect = {};
		GetClientRect(application.GetHwnd(), &rect);

		Viewport viewport(rect);
		Vector3 worldPos = viewport.Unproject(pos, Projection, View, Matrix::Identity);
